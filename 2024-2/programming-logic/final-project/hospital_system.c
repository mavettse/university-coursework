...
void imprimirTodasConsultas(SCHEDULED *scheduled);
...
int main(void)
{
    ...
    memset(patient, 0, 50 * sizeof(PATIENT));
...
void lerarquivos(FILE *fileDoctor, FILE *filePatient, FILE *fileScheduled, DOCTOR *doctor, PATIENT *patient, SCHEDULED *scheduled)
{
    ...
    while (fscanf(fileDoctor, " %[^;];%[^;];%[^;];", doctor[i].dcIdentification, doctor[i].name, doctor[i].specialization) == 3)
    {
        i++;
    }
    ...
    while (fscanf(filePatient, " %[^;];%[^;];%[^;];%[^;];%[^;];%[^;];%[^;];%[^;];", patient[i].paIdentification, patient[i].name, patient[i].id,
                  patient[i].address.street, patient[i].address.number, patient[i].address.city,
                  patient[i].phone, patient[i].gender) == 8)
    {
        i++;
    }
    ...
    while (fscanf(fileScheduled, "%d;%[^;];%[^;];%d:%d;%d/%d/%d;%d:%d", &scheduled[i].num, scheduled[i].dcIdentification, scheduled[i].paIdentification,
...
void menuPesquisarConsulta(DOCTOR *doctor, PATIENT *patient, SCHEDULED *scheduled)
{
    ...
    do
    {
        ...
        case 4:
            ...
            break;
        case 5:
            imprimirTodasConsultas(scheduled);
            break;
        case 6:
            printf("Voltando\n");
            break;
...
void save(DOCTOR *doctor, PATIENT *patient, SCHEDULED *scheduled)
{
    ...
    while (doctor[i].dcIdentification[0] != '\0')
    {
        // Remover caracteres inválidos (opcional)
        doctor[i].name[strcspn(doctor[i].name, ";\n")] = '\0';
        doctor[i].specialization[strcspn(doctor[i].specialization, ";\n")] = '\0';

        fprintf(fileDoctor, "%s;%s;%s;\n",
                doctor[i].dcIdentification,
                doctor[i].name,
    ...
    while (patient[i].paIdentification[0] != '\0' && i < 50)
    {
        fprintf(filePatient, "%s;%s;%s;%s;%s;%s;%s;%s;\n", patient[i].paIdentification, patient[i].name, patient[i].id,
                patient[i].address.street, patient[i].address.number, patient[i].address.city,
...
void incluirMedico(DOCTOR *doctor)
{
    ...
    fgets(doctor[i].specialization, 50, stdin);
    doctor[i].specialization[strcspn(doctor[i].specialization, "\n")] = '\0';
...
void pesquisarMedico(DOCTOR *doctor)
{
    ...
    printf("Identificação do médico(0 para listar todos): ");
    scanf("%s", id);
    getchar();
    ...
    while (doctor[i].dcIdentification[0] != '\0' && strcmp(id, "0") != 0)
    {
        if (strcmp(doctor[i].dcIdentification, id) == 0)
        {
            printf("Nome: %s\n", doctor[i].name);
...
void incluirPaciente(PATIENT *patient)
{
    ...
    printf("Endereço do paciente (Número): ");
    fgets(patient[i].address.number, 50, stdin);
    patient[i].address.number[strcspn(patient[i].address.number, "\n")] = '\0';
...
void alterarPaciente(PATIENT *patient)
{
    ...
    while (patient[i].paIdentification[0] != '\0')
    {
        if (strcmp(patient[i].paIdentification, id) == 0)
        {
            ...
            if (choice == 1)
            {
                printf("ID: %s\nNome: %s\nIdentidade: %s\nEndereço: Rua %s, %s %s\nTelefone %s\nSexo: %s\n\n", patient[i].paIdentification, patient[i].name, patient[i].id, patient[i].address.street, patient[i].address.number, patient[i].address.city, patient[i].phone, patient[i].gender);
            }
...
void pesquisarPaciente(PATIENT *patient)
{
    ...
    printf("Identificação do paciente(0 para listar todos): ");
    scanf("%s", id);
    getchar();
    ...
    if (strcmp(id, "0") == 0)
    {
        ...
        for (i = 0; patient[i].name[0] != '\0'; ++i)
        {
            printf("ID: %s\nNome: %s\nIdentidade: %s\nEndereço: Rua %s, %s %s\nTelefone %s\nSexo: %s\n\n", patient[i].paIdentification, patient[i].name, patient[i].id, patient[i].address.street, patient[i].address.number, patient[i].address.city, patient[i].phone, patient[i].gender);
        }
    }

    while (patient[i].paIdentification[0] != '\0' && strcmp(id, "0") != 0)
    {
        if (strcmp(patient[i].paIdentification, id) == 0)
        {
            printf("Nome: %s\n", patient[i].name);
